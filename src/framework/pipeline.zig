...
pub fn RenderPipeline(comptime T: type) type {
    return struct {
        ...
        pub fn addRenderPass(self: *Self, handler: RenderFn) !void {
            try self.passes.append(self.allocator, .{ .exec = handler });
        }
        ...
        pub fn execute(self: Self, f: *const Frame, ctx: RenderContext) !void {
            for (self.passes.items) |pass| try pass.exec(ctx, f, self.userCtx);
        }

        /// Erase type so it can be stored as Pipeline.
